...
class TestCLIMcpConfiguration:
    ...
    def test_codex_install_surfaces_missing_mcp_servers(self, tmp_path, monkeypatch):
        monkeypatch.setattr(Manifest, "DEFAULT_PATH", tmp_path / ".bb-skills" / "manifest.json")
        skills_dir = tmp_path / "skills"
        ...
        app = create_app(skills_dir=skills_dir)
        result = runner.invoke(app, ["install", "testing", "--platform", "codex"], env={"HOME": str(tmp_path)})

        assert result.exit_code == 0
        assert "Codex CLI:" in result.output
        assert "Required MCP server(s) not configured: playwright" in result.output
        assert "stdin is non-interactive" in result.output
